...
本项目提供完整的实战案例，展示如何使用 CAD 技能包完成真实工程设计。

### U 形支架冲压模具设计

📁 **目录**: `examples/u-bracket-die/`

**项目简介**: U 形支架冲压工艺及弯曲模具设计自动化脚本，使用 AutoCAD COM 自动化（Python + pywin32）生成。

**功能特点**:
- ✅ 参数化设计 — 所有尺寸在 `Spec` dataclass 中定义
- ✅ 自动绘图 — 生成毛坯展开、凸模、凹模块、总装草图的 DWG 文件
- ✅ 设计报告 — 自动生成 MD 和 DOCX 格式的设计报告
- ✅ 图层管理 — 完整的图层系统（PART, DIE, DIM 等）
- ✅ COM 重试机制 — 优雅地处理 AutoCAD COM 调用失败
...
| `脚本/generate_u_bracket_design_report.py` | 生成 MD 和 DOCX 格式的设计报告 |
...
**注意事项**:
- 本案例为**课程设计验证版**，不适合直接用于生产加工
- 毛坯展开长度采用中性层半径 Rn = R + 0.5t（课程设计近似算法）
- 如需正式使用，请复核材料参数、回弹补偿和模具间隙
